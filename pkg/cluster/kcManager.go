...
func CheckIfConnectionsExist(ctx context.Context, c *KubernetesCluster, namespace string, k *kafkamgmtclient.KafkaRequest) error {
	...
	if data.Error() == nil {
		return fmt.Errorf("%v: %s", c.localizer.LocalizeByID("cluster.kubernetes.checkIfConnectionExist.existError"), k.GetName())
	}
...
func watchForKafkaStatus(c *KubernetesCluster, crName string, namespace string) error {
	c.logger.Info(c.localizer.LocalizeByID("cluster.kubernetes.watchForKafkaStatus.log.info.wait"))
	...
	fmt.Fprint(c.io.Out, c.localizer.LocalizeByID("cluster.kubernetes.watchForKafkaStatus.binding", templateEntries...))
	...
	for {
			...
			if event.Type == watch.Modified {
				...
				if found {
					for _, condition := range conditions {
						...
						if !ok {
							return fmt.Errorf(c.localizer.LocalizeByID("cluster.kubernetes.watchForKafkaStatus.error.format"), typedCondition)
						}
						if typedCondition["type"].(string) == "Finished" {
							if typedCondition["status"].(string) == "False" {
								w.Stop()
								return fmt.Errorf(c.localizer.LocalizeByID("cluster.kubernetes.watchForKafkaStatus.error.status"), typedCondition["message"])
							}
							if typedCondition["status"].(string) == "True" {
								c.logger.Info(c.localizer.LocalizeByID("cluster.kubernetes.watchForKafkaStatus.log.info.success", localize.NewEntry("Name", crName), localize.NewEntry("Namespace", namespace)))
		...
		case <-time.After(60 * time.Second):
			w.Stop()
			return fmt.Errorf(c.localizer.LocalizeByID("cluster.kubernetes.watchForKafkaStatus.error.timeout"))
		}
	}
}
